# 2026-09-14 · Flat palm push ups cost me a month

**Program:** Day 2 · Week 1 · Phase 1 Foundation and Ignition
**Session:** Conditioning A - Engine
**Pillar:** Teaching
**Format:** Hard Lesson
**Target length:** 35 sec
**Status:** ready to film
**Series:** 6 · Big Guy Beginner, episode 2

---

## Hook · 0 to 3 sec

**Say:** "Flat palm push ups wrecked my left wrist."
**On screen:** THE WRIST MISTAKE

## Beats

| Time | Say | On screen | Camera |
|---|---|---|---|
| 0 to 3 s | Flat palm push ups wrecked my left wrist. | THE WRIST MISTAKE | Close on the wrist, flexing it slowly |
| 3 to 12 s | At 258 pounds, every push up puts most of me through two joints that never signed up for it. Mine complained. I kept going. That is the actual mistake. | 258 LB THROUGH TWO WRISTS | Demo the flat palm position, then stop short of loading it |
| 12 to 24 s | Now I press on my fists or on bars. Same muscles, neutral wrist, no bend under load. Pike push ups on parallettes instead of the floor. Dips shallow until they are pain free. | FISTS · BARS · PARALLETTES | Three quick demo clips, each one held long enough to copy |
| 24 to 32 s | I did not train around it to be careful. I trained around it because a month off costs more than any single session is worth. | A MONTH OFF COSTS MORE | Mid shot, on the bike mid session, still moving |
| 32 to 35 s | If something hurts every rep, the answer is a different angle, not a longer warm up. | | Straight to lens |

## Close

**Say:** "If something hurts every rep, the answer is a different angle, not a longer warm up."
**CTA:** none - stage 0, no asks
**Signature line:** not this one

---

## Caption

Not a doctor, not advice. This is what I do so I can train six days a week.

## Hashtags

#calisthenics #wristpain #pushups #beginnerfitness

## Cover frame

The fist position mid push up, text: STOP DOING THIS

## Sound

Original audio.

## Shot list

- [ ] Close on the wrist, flexing, 3 sec
- [ ] Flat palm position demo, no load, 4 sec
- [ ] Fist push up, 4 sec
- [ ] Pike push up on bars, 4 sec
- [ ] Shallow dip, 3 sec
- [ ] Talking beats, 12 sec total

## Numbers to confirm before filming

- [ ] none

## Notes

Engine day is quiet, so this is the talking slot of the week. Describe the injury,
never diagnose it. No "this is tendinitis," no "this fixes carpal tunnel."
