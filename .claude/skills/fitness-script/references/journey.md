# The journey

The factual spine of the account. Every script traces its numbers here. Update this
file whenever the tracker updates, and date every entry.

## Source of truth

Real numbers live in the tracker: `assets/app.js` (seeded baselines and program
config) and browser storage for everything logged since. If a number is not in one of
those places, it goes into a script as a bracket for Saki to fill, never as a guess.

## Where the story starts

| | |
|---|---|
| Program | Foundation Protocol - heavy calisthenics and Marine conditioning, self coached |
| Day 1 | 2026-09-13 |
| Length | Six months, reassessed every four weeks |
| Phases | 1 Foundation and Ignition (wk 1 to 4) · 2 Base Building (wk 5 to 10) · 3 Marine Conditioning Build (wk 11 to 16) · 4 Performance and Tactical Fitness (wk 17 to 24) |
| Training days | Six, one true rest day |

## Day 1 is 2026-09-13

An earlier block was started 2026-08-16 and the baselines below were tested that day, but
not one session of it was logged in the tracker. Saki called 2026-09-13 day one for
everything: the program, the measurements, the content, and the club. That is the date the
account counts from.

Say "never logged," not "never trained," unless he confirms he did not train. The tracker
shows no sessions. It does not show what happened.

## Baselines, tested 2026-08-16

The honest starting line, and the most valuable content on the account - nobody posts a
real day one. These stand as the reference numbers until they are retested, and day 1
retests them on camera, which is the single best use of a first day.

| Test | Last tested 2026-08-16 |
|---|---|
| Push ups, strict max | 10 |
| Pull ups | 0 |
| Dead hang | 10 sec |
| Hollow hold | 25 sec |
| Wall sit | 75 sec |
| One mile run | 11.8 min |
| 2,000 m row | 8.4 min |
| Smith bench, 5 reps | 125 lb |
| Dumbbell bench, 5 reps | 50 lb |
| Dumbbell shoulder press, 5 reps | 40 lb |
| Lat pulldown, 5 reps | 100 lb |
| Seated cable row, 5 reps | 135 lb |
| Romanian deadlift, 5 reps | 115 lb |
| Leg extension, 8 reps | 80 lb |
| Leg curl, 8 reps | 140 lb |
| Biceps curl machine, 8 reps | 100 lb |
| Vertical crunch, max | 100 reps |
| Steps baseline | 2,784 per day |

## Bodyweight

| Date | Weight | Source |
|---|---|---|
| 2026-08-21 | 256.4 lb | tracker, earliest logged weigh in |
| 2026-09-13 | 257.6 lb | measurement panel |

Two data points in four weeks is not a trend, it is two numbers. **Weigh in weekly, same
morning, same conditions, and log it** - otherwise the account cannot tell a story about
the scale, and neither can he. This is the single highest value habit to add right now and
it costs thirty seconds.

The +1.2 lb does not get hidden, spun, or explained with a mechanism nobody measured. It
gets posted, the plan stays the same, and the next weigh in is the answer. No claims about
water, glycogen, or muscle gain unless something actually measured them.

Only logged weigh ins go in a video. If the tracker has no entry for this week, the
script says `[WEIGHT TODAY]` and he fills it at the camera or the beat gets cut.

## Measurement panel · 2026-09-13

Day 1. The full panel, and the before set for every comparison from here on. The scale was
never going to be enough on its own and this is the fix.

| Measure | 2026-09-13 |
|---|---|
| Height | 6 ft |
| Weight | 257.6 lb |
| Chest | 125.5 cm |
| Underbust | 112 cm |
| Abs | 113.5 cm |
| Waist | 118.5 cm |
| Hips | 118 cm |
| Butt | 124.5 cm |
| Thigh | R 76.5 cm · L 73 cm |
| Upper arm | R 44.5 cm · L 41 cm |
| Calf | R 47 cm · L 47.5 cm |

257.6 lb is the day 1 number and the one every future weigh in gets measured against. The
only earlier logged weight is 256.4 lb on 2026-08-21, three weeks before, which makes the
honest read **up 1.2 lb over 23 days** on two data points. Not a trend. Two numbers.

The asymmetries are worth knowing and worth saying on camera: right thigh 3.5 cm bigger
than left, right arm 3.5 cm bigger than left, left calf 0.5 cm bigger than right. That is
a real thing to train and a real thing to talk about, and it is the kind of detail that
makes a viewer trust that the numbers were actually measured.

Retest the full panel at every phase change. Same tape, same time of day, same
conditions, or the comparison is worthless. Next panel: start of Phase 3, week 11.

The tape matters more than the scale here. At 257 lb, training six days a week, the waist
can drop while the scale sits still, and that is the exact situation where most people
quit. The panel is the insurance against quitting over one number.

## Daily non negotiables

Step target, ten minutes of mobility, seven to nine hours of sleep, 180 to 200 g
protein, 150 to 180 oz water. Calories 2,100 to 2,250 in phases 1 and 2, fat 70 to
80 g floor.

## The body, honestly

Left wrist does not tolerate flat palm loading, so pressing runs on fists,
parallettes, or bars. Knees get protected on running progression - rowing and biking
replace runs whenever knees, shins, or hips complain. These are not weaknesses to hide
in the content. They are the reason the plan is smart, and they are what makes it
believable to a 250 lb viewer who also has something that hurts.

## The arc the account is documenting

Day 1, 2026-09-13: 258 lb, zero pull ups, ten push ups, a ten second dead hang, and a
mile last timed at 11.8 minutes.

Where it is going: first pull up, sub nine minute mile, sub eight, eventually a mile
under six. Loaded rucks at 25 to 35 lb. A Marine PT test standard. The weight is a
side effect of the standard, not the point.

Named milestones worth their own video, in the order they will probably arrive:

1. First unassisted pull up
2. 30 second dead hang
3. Under 250 lb
4. Sub nine minute mile
5. 60 second hollow hold
6. 25 push ups strict
7. Under 240 lb
8. First 35 lb ruck, full hour
9. Sub eight minute mile
10. First full PT test attempt

## Date Miles Club

Written, dated to start in 2026, never stuck to. Launches 2026-10-01. It is the offer the whole account is
being built toward and it is also the account's best piece of honest content. The charter,
the math on why it did not stick, and the fixed version are in `date-miles-club.md`.

## Chapter marks

| Mark | Date | What it is |
|---|---|---|
| Baselines tested | 2026-08-16 | An unlogged block, the numbers still stand |
| Day 1 | 2026-09-13 | Panel taken, series slate filmed, the count starts |
| Date Miles Club day 1 | 2026-10-01 | One mile, member number one |
| Phase 2 opens | week 5 | Volume climbs, pull up progression gets serious, run intervals begin |
| Phase 3 opens | week 11 | Rucks, carries, PT circuits, tempo running |
| Phase 4 opens | week 17 | Advanced calisthenics, plyos, speed work |
| Six month retest | week 24, about 2027-03-14 | Every day 1 number retested on camera, one video |

The week 24 retest video is the single most important video this account will post in
its first year. Every day one number on screen next to the new one. Film the baselines
again with the same framing as day one so the cut is honest.

## What is off limits

No claimed Marine affiliation, service, or credential. The program is Marine style
conditioning, self coached, and that is how it always gets said.

No coaching advice pitched as certified until personal training is actually live and
credentialed. Until then the content is "here is what I am doing," never "here is what
you should do."

No medical claims. Injuries get described, never diagnosed.
